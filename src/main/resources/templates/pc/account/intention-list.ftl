...
<head>
    ...
    <link rel="stylesheet" href="${css("pc/intention_list.css")}">

</head>
<body>
<div class="page-box intention">
  <#include "../include/header_page.ftl">
    <div class="back">
        <p onclick="_hmt.push(['_trackEvent', '返回我的账户_pc', 'click', 'source=${source!}']);backToAccount()" >
            <span><img src="${image("pc/account/arrow-left.png")}"/>返回我的账户</span>
        </p>
    </div>
    ...
    <div class="intention-content">
        ...
        <p class="title">保时捷 Taycan 购车意向单</p>
        <p class="title t-bottom">条款与条件</p>
        <span class="small-btn" onclick="_hmt.push(['_trackEvent', '下载购车意向单_pc', 'click', 'source=${source!}']);pdfDownload('${order.orderSn}')" >下载购车意向单</span>
        <article class="list">
